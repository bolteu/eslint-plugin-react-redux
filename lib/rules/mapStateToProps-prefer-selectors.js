...
const checkProperties = function (context, properties, matching, expectedParams) {
  properties.forEach((prop) => {
    ...
    if (prop.key.type === 'Identifier' && prop.value.type === 'Identifier' && prop.key.name === prop.value.name) {
      // While this is a legitimate loophole to avoid any kind of plugin errors,
      // this creates a lot of false positives if mapStateToProps has beefed up with some logic
      return;
    }
    ...
    // shouldScrollToTop: navigation.getParam('shouldScrollToTop')
    // This can technically be a selector through "import * as navigation from './redux/selectors"
    // But reporting it as not selector is better than crashing
