...
describe('configurations', () => {
  it('should export a \'recommended\' configuration', () => {
    ...
    assert(plugin.configs.recommended.plugins.includes('@bolteu/react-redux'));
    Object.keys(plugin.configs.recommended.rules).forEach((configName) => {
      assert.equal(configName.indexOf('@bolteu/react-redux/'), 0);
      const ruleName = configName.substring('@bolteu/react-redux/'.length);
      assert(plugin.rules[ruleName]);
    });
  });
  it('should export a \'all\' configuration', () => {
    assert(plugin.configs.all);
    assert(plugin.configs.all.plugins.includes('@bolteu/react-redux'));
    ...
    ruleFiles.forEach((ruleName) => {
      ...
      const inAllConfig = Boolean(plugin.configs.all.rules[`@bolteu/react-redux/${ruleName}`]);
